...
| 💾 **布局持久化** | 模块位置、连线中转点、平移缩放状态按设计名保存至 `localStorage` 及服务端 JSON；重启服务后自动恢复最近打开的设计 |
...
| 📝 **持久注释块** | 框选区域可转换为半透明圆角注释块；仅显示标题，右键编辑 Markdown 详情；位置、大小、内容随设计 JSON 保存 |
| 🧩 **多选与补全选择** | 单击选中模块，`Shift+单击` 连续增删多选，`Ctrl+单击` 自动补全包围框内模块/拐点；选中后支持拖动和 WASD 微调 |
| 📊 **加载进度提示** | 打开大型设计时按阶段显示进度，避免初次渲染和状态恢复时界面看起来无响应 |
...
2. **浏览模块层次** — 左侧树的三角按钮只控制树层级展开/收起，节点正文单击只定位并高亮当前画布中的实例，节点正文双击才进入独立模块视图。画布模块右上角 `▶` 可在当前位置递归展开，`▼` 收起，双击模块则进入独立模块视图。展开后可把内部模块自由拖出当前边界，父级和祖先外框会向对应方向自动扩展并继续包围内容。
...
10. **多选模块** — 单击模块选中单个模块；`Shift+单击` 连续加入 / 移出多个模块；`Ctrl+单击` 将当前选区与目标模块拟合为外框，并自动选中框内模块和线拐点。选中后可拖动选框内部空白或使用 `W/A/S/D` 微调。
11. **注释块** — `Shift+空白拖拽` 框选模块 / 拐点后点击 **注** 可转换为永久注释块；注释块可拖动、缩放，右键打开两级设置：第一页 Markdown 预览，第二页 Markdown 源码和导入。`Shift+拖拽注释块` 会带着框内模块 / 拐点一起移动。
12. **快捷键说明** — 按 `H`（默认）或点击工具栏 **说明** 按钮切换弹窗。弹窗分「可自定义按键」和「快捷键一览」两区，可直接调整移动速度、缩放步进并重绑定所有可自定义键，设置实时保存至设计 JSON。
